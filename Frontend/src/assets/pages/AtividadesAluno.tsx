  ...
  return (
    <div className="w-full h-auto p-4 sm:p-6 lg:p-8 bg-white rounded-xl shadow-md flex flex-col gap-4">
      <div className="space-y-6">
        ...
        <div className="flex overflow-x-auto pb-2 sm:pb-0 sm:flex-wrap gap-2 sm:gap-3 mb-4 no-scrollbar">
          {(["Todos", "PENDENTE", "ENTREGUE", "EXPIRADO"] as const).map(
            (tipo) => (
              <button
                key={tipo}
                onClick={() => setFilter(tipo)}
                className={`px-4 sm:px-6 h-9 rounded-lg font-bold transition-all whitespace-nowrap text-sm sm:text-base ${
                  filter === tipo
        ...
        <div className="grid grid-cols-1 gap-4">
          {atividadesFiltradas.map((a) => (
            <div
              key={a.id}
              className="border rounded-lg p-4 sm:p-5 bg-white shadow-sm hover:shadow-md transition-shadow flex flex-col"
            >
              <div className="flex flex-col sm:flex-row justify-between items-start gap-2">
                <div className="flex-1">
                  <h2 className="font-bold text-base sm:text-lg text-gray-800 leading-tight">
                    {a.titulo}
                  </h2>
                  <p className="text-xs sm:text-sm font-medium text-[#1D5D7F]">
                    {a.disciplina}
                  </p>
                </div>

                <span
                  className={`px-3 py-1 rounded-full text-[10px] sm:text-xs font-bold uppercase self-start sm:self-center ${
                    a.status === "ENTREGUE"
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mt-4 pt-4 border-t border-gray-100 gap-3">
                ...
                {a.nota !== null && (
                  <div className="text-xs sm:text-sm bg-green-50 px-3 py-1 rounded-lg border border-green-100">
              ...
              <button
                ...
                className={`mt-4 w-full py-2 sm:py-2.5 rounded-lg font-bold text-sm sm:text-base transition-colors ${
                  a.status === "ENTREGUE"
